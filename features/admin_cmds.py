...
class AdminCommands(discord.Cog):
    ...
    blocklist_subcommand = admin_subcommand.create_subgroup(
        name="blocklist", description="Manage the blocklist", guild_ids=[ADMIN_GUILD])
    ...
    async def blocklist_remove_user(self, ctx: discord.Interaction, user: discord.User):
        ...
        await ctx.response.send_message(f"User {user.mention} has been removed from the blocklist", guild_ids=[ADMIN_GUILD])
